...
app.get('/api/monetag-postback', async (req, res) => {
    ...
    // Calcular los puntos dinámicos basados en la ganancia real enviada por Monetag
...
app.post('/api/v1/web-video/claim', verifyToken, rewardRateLimit, async (req, res) => {
    ...
    try {
        ...
        // Obtener los puntos reales registrados en el postback de Monetag
        const realPoints = Number(session.points_awarded) || 0;
        if (realPoints <= 0) {
            await safeRollback(client);
            return res.status(400).json({ success: false, error: 'No se detectaron ingresos para esta recompensa.' });
        }
