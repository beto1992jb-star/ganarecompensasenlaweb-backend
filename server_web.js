...
// ============================================================
// MONETAG
// Debe coincidir con index.html
// ============================================================

const MONETAG_ZONE_ID = process.env.MONETAG_ZONE_ID || '268402';

const MONETAG_SDK_URL =
    process.env.MONETAG_SDK_URL ||
    'https://quge5.com/88/tag.min.js';

const MONETAG_DIRECT_LINK_URL =
    process.env.MONETAG_DIRECT_LINK_URL ||
    'https://quge5.com/88/tag.min.js';
...
// Recompensa fija de respaldo si Monetag no informa payout.
// Si Monetag confirma un payout válido, se utiliza ese valor.
...
// Tiempo mínimo antes de poder reclamar.
...
// Límite de seguridad para un payout individual de Monetag.
// Evita que un valor malformado o inesperado genere millones de puntos.
const MAX_MONETAG_REWARD_POINTS = 100000;
...
app.get('/', (req, res) => {
    ...
    return res.status(200).json({
        success: true,
        message:
            'API Backend de GanaRecompensasEnLaWeb activa y funcionando 🚀',
        version: '1.0.0'
    });
});
...
app.get(
    ...
    async (req, res) => {
        ...
        // Si configuraste un secreto,
        // Monetag debe enviarlo correctamente.
        if (
            MONETAG_SECRET &&
        ...
        try {
            ...
            if (result.rows.length === 0) {
                console.warn(
                    'Postback Monetag recibido para sesión inexistente, expirada o ya reclamada:',
                    String(sessionId)
                );
            }
            ...
            console.error(
                'Error procesando postback Monetag:',
                err
            );
        ...
        try {
            ...
            if (!myReferralCode) {
                ...
                return res.status(500).json({
                    success: false,
                    error:
                        'No se pudo generar el código de referido.'
                });
            }
...
app.post(
    ...
    async (req, res) => {
        ...
        return res.json({
            success: true,
            message:
                'Si el correo electrónico se encuentra registrado, recibirás las instrucciones para restablecer tu contraseña.'
        });
    }
);
...
// ============================================================
// MONETAG VIDEO - STATUS
// ============================================================

app.get(
    '/api/v1/web-video/status',
    verifyToken,
    async (req, res) => {
        const userId =
            req.user.userId;
        ...
        try {
            ...
            return res.json({
                ...
                claimed:
                    activeSession
                        ? !!activeSession.claimed_at
                        : false,

                confirmed:
                    activeSession
                        ? !!activeSession.confirmed_at
                        : false,
        ...
        try {
            ...
            if (
                ...
                return res.status(400).json({
                    success: false,
                    error:
                        'La sesión de recompensa expiró.'
                });
            }

            // Si existe MONETAG_SECRET,
            // exigimos confirmación real del postback.
            if (
                MONETAG_SECRET &&
                ...
                return res.status(400).json({
                    ...
                    errorCode:
                        'WAITING_MONETAG_CONFIRMATION',

                    error:
                        'Esperando confirmación del servidor de Monetag...'
                });
            }
            ...
            return res.json({
                ...
                cooldownSeconds:
                    VIDEO_COOLDOWN_MS / 1000,

                message:
                    `¡Puntos otorgados con éxito! Sumaste ${realPoints} pts.`
            });
        } catch (error) {
            console.error(
                'Error reclamando video:',
                error
            );
            ...
            if (
                ...
                if (
                    ...
                    return res.status(429).json({
                        success: false,
                        error:
                            `Esperá ${remaining} segundos para iniciar otra sesión de juego.`
                    });
                }
            }
        ...
        try {
            ...
            if (
                ...
                return res.status(400).json({
                    success: false,
                    error:
                        `Todavía no pasaron ${GAME_MIN_SECONDS} segundos de juego.`
                });
            }
            ...
            return res.json({
                ...
                cooldownSeconds:
                    GAME_COOLDOWN_MS / 1000
            });
        } catch (error) {
            console.error(
                'Error reclamando juego:',
                error
            );
...
const processWithdrawal =
    async (req, res) => {
        ...
        if (
            typeof payoutDestination !==
                'string' ||
            payoutDestination
                .trim()
                .length === 0 ||
            payoutDestination.length > 255
        ) {
            return res.status(400).json({
                success: false,
                error:
                    'Destino de retiro inválido.'
            });
        }
        ...
        if (
            ...
            return res.status(400).json({
                success: false,
                error:
                    `El monto mínimo para ${normalizedMethod} es de $${config.minAmount.toFixed(2)} USD.`
            });
        }
        ...
        if (
            ...
            return res.status(400).json({
                success: false,
                error:
                    'No se pudo calcular correctamente el retiro.'
            });
        }
        ...
        try {
            ...
            // Los puntos se bloquean inmediatamente
            // mientras el retiro está pendiente.
            await client.query(
                `
                UPDATE web_users
                SET points_balance =
            ...
            return res.json({
                ...
                message:
                    'Solicitud de retiro registrada exitosamente.'
            });
        } catch (error) {
            console.error(
                'Error withdrawal:',
        ...
        try {
            ...
            if (
                ...
                return res.status(400).json({
                    success: false,
                    error:
                        'El retiro ya fue finalizado previamente.'
                });
            }
            ...
            if (
                ...
                // Solamente devolvemos los puntos si
                // el retiro estaba pendiente o aprobado.
                // Como los estados rechazado/completado
                // ya fueron bloqueados arriba, la devolución
...
// ============================================================
// MAPEO DE COMPATIBILIDAD DE RUTAS
// ============================================================
...
async function startServer() {
    try {
        ...
        app.listen(
            ...
            () => {
                console.log(
                    `Servidor iniciado y escuchando en el puerto ${PORT}`
                );

                console.log(
                    `Monetag Zone ID: ${MONETAG_ZONE_ID}`
                );
